// CANSAT PROGRAM : 2022.05.13 hypark@satrec (New BT Configuration)
// CANSAT PROGRAM : 2022.05.03 hypark@satrec (New BT)
// CANSAT PROGRAM : 2022.04.04 hypark@satrec (New Gpsr)
...
#define WAIT_CNT_BT_NEW_STEP        0x100000


// Payload 데이터를 얻기 위해 UART 포트의 수신 속도 및 MUX를 설정한다.
#define CAMERA_INIT_BAUD 38400
//#define CAMERA_BAUD 38400
#define CAMERA_BAUD     115200
#define IMU_BAUD        115200
#define GPS_BAUD          9600
#define ATLM_BAUD  CAMERA_BAUD
#define DTLM_BAUD  CAMERA_BAUD
...
void hyRxMode_Set(unsigned char iMode)
{
	switch(iMode)
	{
		...
		case RxMode_CAMERA_IMAGE: PL_PORT_SPEED_SET(CAMERA_BAUD) ; hyRxMode = iMode ; hyRxMode_PortSet(RxMode_CAMERA_PORT) ; break;
		case RxMode_IMU         : PL_PORT_SPEED_SET(IMU_BAUD)    ; hyRxMode = iMode ; hyRxMode_PortSet(RxMode_IMU_PORT)    ; break;
		case RxMode_GPS         : PL_PORT_SPEED_SET(GPS_BAUD)    ; hyRxMode = iMode ; hyRxMode_PortSet(RxMode_GPS_PORT)    ; break;
		case RxMode_ATLM        : PL_PORT_SPEED_SET(ATLM_BAUD)   ; hyRxMode = iMode ; break;
		case RxMode_DTLM        : PL_PORT_SPEED_SET(DTLM_BAUD)   ; hyRxMode = iMode ; break;
...
// 2.1 BT 통신 속도 초기화  : AT+BTUART,115200,N,1
...
void hyBT_NEW_CFG_CHAR(unsigned char ch)
{
  hyGsSetTxByte(ch);
  hyGsSetTxByte(0x0D);
}

void hyBluetooth_New_Cfg()
{
  hyBT_NEW_CFG_CHAR('t');hyTransUartWait(WAIT_CNT_BT_STEP) ; // TOP
  hyBT_NEW_CFG_CHAR('t');hyTransUartWait(WAIT_CNT_BT_STEP) ; // TOP
  hyBT_NEW_CFG_CHAR('3');hyTransUartWait(WAIT_CNT_BT_STEP) ; // MODE
  hyBT_NEW_CFG_CHAR('2');hyTransUartWait(WAIT_CNT_BT_STEP) ; // MODE 2  
  hyBT_NEW_CFG_CHAR('5');hyTransUartWait(WAIT_CNT_BT_STEP) ; // UART
  hyBT_NEW_CFG_CHAR('1');hyTransUartWait(WAIT_CNT_BT_STEP) ; // BAUD RATE
  hyBT_NEW_CFG_CHAR('8');hyTransUartWait(WAIT_CNT_BT_STEP) ; // 115200
  hyBT_NEW_CFG_CHAR('t');hyTransUartWait(WAIT_CNT_BT_STEP) ; // TOP  
  hyBT_NEW_CFG_CHAR('6');hyTransUartWait(WAIT_CNT_BT_STEP) ; // ROLE
  hyBT_NEW_CFG_CHAR('1');hyTransUartWait(WAIT_CNT_BT_STEP) ; // SLAVE
  hyBT_NEW_CFG_CHAR('7');hyTransUartWait(WAIT_CNT_BT_STEP) ; // OPMODE
  hyBT_NEW_CFG_CHAR('1');hyTransUartWait(WAIT_CNT_BT_STEP) ; // OPMODE 0
  
//hyBT_NEW_CFG_CHAR(0x08);hyTransUartWait(WAIT_CNT_BT_STEP) ; // Data Input Cancel
}
...
void hyBluetooth_Init()
{
  ...
  hyBluetooth_New_Cfg() ; // set Configuration 
  hyTransUartWait(WAIT_CNT_BT_STEP) ;
  
  hyBluetooth_Org_Init(); // set with 9600
	
//	hyBT_STOP()  ;  hyTransUartWait(WAIT_CNT_BT_STEP) ;
//	hyBT_MODE()  ;	hyTransUartWait(WAIT_CNT_BT_STEP) ;
//	hyBT_115200();	hyTransUartWait(WAIT_CNT_BT_STEP) ;
//  hyBT_New_115200();  hyTransUartWait(WAIT_CNT_BT_STEP) ;
...
void hyCamera_Init()
{
  ...
  PL_PORT_SPEED_SET(CAMERA_INIT_BAUD) ;
  GS_PORT_SPEED_SET(GS_TX_SPEED);
  
  hyTransUartWait(WAIT_CNT_CAMERA_INIT_STEP);
	hyCamera_Reset()         ;	hyTransUartWait(WAIT_CNT_CAMERA_INIT_STEP); 
	hyTransUartWait(WAIT_CNT_CAMERA_INIT_STEP);
//	hyCamera_PictureSize()   ;	
	hyTransUartWait(WAIT_CNT_CAMERA_INIT_STEP);
	for( unsigned char i = 0 ; i < 10 ; i++ )
	{
#if CAMERA_BAUD == 9600
    hyCamera_9600_bps();
#elif CAMERA_BAUD == 38400
		hyCamera_38400_bps();
#else
    hyCamera_115200_bps();
#endif     
...
// 76 00 MSG_TYPE MSG_LEN MSG //
unsigned char msg_data[96];
unsigned char msg_data_idx=0;
unsigned char msg_recv_flag=0;
void hyMsg_RecvByte(unsigned char b)
{
  static unsigned char msg_state=0 ;
  static unsigned char msg_type =0 ;
  static unsigned char msg_len  =0 ;
  unsigned char idx;

  switch( msg_state )
  {
    case 0 : msg_state = ( b == 0x76 ) ? 1 : 0 ;                        break;  // 0x76
    case 1 : msg_state = ( b == 0x00 ) ? 2 : (( b == 0x76 ) ? 1 : 0);   break;  // 0x00
    case 2 : msg_state = ( 0xA0 <= b && b <= 0xAF ) ? 3 : (( b == 0x76 ) ? 1 : 0) ; msg_type = b ;  break;  // MSG_TYPE
    case 3 : msg_state = 4; msg_len = ( b < 96) ? b : 95 ; msg_data_idx = 0 ;                       break;  // MSG_LEN
    case 4 : if( msg_data_idx < msg_len  )  { msg_data[msg_data_idx] = b ; msg_data_idx++ ; } 
             if( msg_len <= msg_data_idx ) 
             { 
                msg_recv_flag = 1; 
                Gs_Tx_QueueIn(0x76);
                Gs_Tx_QueueIn(0x00);
                Gs_Tx_QueueIn(msg_type);
                Gs_Tx_QueueIn(msg_data_idx);
                for( idx = 0 ; idx < msg_data_idx ; idx++ )
                {
                  Gs_Tx_QueueIn(msg_data[idx]);
                }
                msg_state = 0    ; 
             } 
             break;
    default: msg_len = 0 ; break;
  }
}
...
// MSG 메인 동작 함수
// 1. UART 모드 설정
// 2. SubModule 로부터 데이터 획득
// 3. SubModule 데이터 지상 전송

#define WAIT_CNT_MSG        0x020000
void hyCANSAT_MSG_OPERATION(unsigned char iRxMode)
{
  msg_recv_flag = 0 ;
  hyRxMode_Set(iRxMode);
  hyTransUartUntil(&msg_recv_flag,0,WAIT_CNT_MSG );
  hyTransUartWait(WAIT_CNT_SEND);
}


void hyCANSAT_M4_OPERATION() { DDRA = 0xFE ; hyCANSAT_MSG_OPERATION(RxMode_M4); }
void hyCANSAT_M5_OPERATION() { DDRA = 0xFC ; hyCANSAT_MSG_OPERATION(RxMode_M5); }
void hyCANSAT_M6_OPERATION() { DDRA = 0xFD ; hyCANSAT_MSG_OPERATION(RxMode_M6); }
void hyCANSAT_M7_OPERATION() 
{ 
DDRA = 0xF7 ; hyCANSAT_MSG_OPERATION(RxMode_M7);
...
void loop() 
{
 ...
 while(Serial.available()) {
  ...
  hyCANSAT_Gs_Tx_Message(3,"CO2 / Temperatrue");
